...
export class HandlersSubJob implements IHandlersSubJob {
    ...
    constructor (
        ...
        this._job = new CronJob(this._config.cron.interval, () => {
            ...
            const data = this._metrics_store.query(query_record.db, query_record.table, query_record.record);
        ...
        this._logger.log(`[Handlers] Sub job ${chalk.gray(this._id)} created`, "dev");
    ...
    async run (): Promise<void> {
        ...
        this._job.start();
        this._logger.log(`[Handlers] Sub job ${chalk.gray(this._id)} running`, "dev");
    }
